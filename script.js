...
newSubmit.addEventListener('click', e => {
	e.preventDefault();
	if (
		btitle.value.includes('/') ||
		bauthor.value.includes('/') ||
		bpages.value.includes('/')
	) {
		return (displayError.textContent = `the character "/" is not allowed to be used`);
	}
	if (btitle.value === '') {
		return (displayError.textContent = "Book's title was empty");
	} else if (bauthor.value === '') {
		return (displayError.textContent = "Book's author was empty");
	} else if (bpages.value === '') {
		return (displayError.textContent = 'Amount of pages was empty');
	} else {
	...
	localStorage.setItem(
		localStorageIndexCount++,
		`${userTitle}/${userAuthor}/${userPages}`
	);
